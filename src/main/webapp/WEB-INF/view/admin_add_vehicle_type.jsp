...
<div>
    <div class="container-sm mt-5">
        <form:form id="form" method="post" enctype="multipart/form-data" modelAttribute="vehicleTypeForm">
            <div class="col-lg-4 col-md-4 col-sm-4 container justify-content-center">
                <div class="form-floating mb-3">
                    <form:input path="vehicleType" id="vehicleType" maxlength="50" class="form-control" type="text"
                                autofocus="autofocus"/>
                    <label for="vehicleType">Category</label>
                </div>
                <div class="form-floating mb-3">
                    <form:textarea path="description" id="vehicleTypeDescription" style="height: 100px" maxlength="200" class="form-control"
                                   type="text"/>
                    <label for="vehicleTypeDescription">Description</label>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="vehicleTypeImage">Category image</label>
                    <input name="vehicleTypeImage" type="file" class="form-control" id="vehicleTypeImage"/>
                    <div class="form-text">Please insert an image to differentiate the category with others</div>
                </div>
                <div>
                    <button type="submit" class="btn btn-outline-secondary m-auto d-block mb-5">Add category</button>
                </div>
                <div>
...
<script>
    window.onload = function () {
        ...
        if (errorMessage !== "") {
            Swal.fire({
                title: "Error occurred while adding the new category!!!",
                text: errorMessage,
                icon: "error",
            });
        }
    ...
    form.addEventListener('submit', function (event) {
        ...
        const description = $("#vehicleTypeDescription").val();
        const image = $("#vehicleTypeImage").val();

        if (type.length < 2 || type.length > 50) {
            event.preventDefault();
            Swal.fire({
                title: "Error in vehicle category!!!",
                html: '<div> Vehicle category cannot be kept empty </div>' +
                    '<div> <br> Vehicle category should contain 2 to 50 characters </div>',
                icon: "error",
            });
        } else if (description.length < 2 || description.length > 200) {
            event.preventDefault();
            Swal.fire({
                title: "Error in vehicle category description!!!",
                html: '<div> Vehicle category description cannot be kept empty </div>' +
                    '<div> <br> Vehicle category should contain 2 to 200 characters </div>',
                icon: "error",
            });
        } else if (image < 1) {
            event.preventDefault();
            Swal.fire({
                title: "Error in vehicle category image!!!",
                html: '<div> The vehicle category image cannot be kept empty </div>',
                icon: "error",
            });
        } else {
            Swal.fire({
                title: 'Adding...',
                html: 'Hold on a few seconds while we add the new vehicle category!',
                timer: 10000,
